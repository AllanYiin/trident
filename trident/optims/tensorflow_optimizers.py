...
#
# class RAdam(tf.keras.optimizers.Optimizer,OptimizerMixin):
...
#
#     ```python
#     opt = tfa.optims.RectifiedAdam(lr=1e-3)
#     ```
#
...
#
#     ```python
#     radam = tfa.optims.RectifiedAdam()
#     ranger = tfa.optims.Lookahead(radam, sync_period=6, slow_step_size=0.5)
#     ```
#     """
#
#     def __init__(self,
...
class Lookahead(tf.keras.optimizers.Optimizer,OptimizerBase):
    ...
    ```python
    opt = tf.keras.optims.SGD(learning_rate)
    opt = tfa.optims.Lookahead(opt)
    ```
    """
