...
class Model(ModelBase):
    ...
    def do_on_data_received(self, train_data, test_data):
        ...
        if self.training_context['steps'] == 0 and (
                'data_feed' not in self.training_context or len(self.training_context['data_feed']) == 0 or  len([v for v in  self.training_context['data_feed'].value_list if v is None])>0):
            try:
