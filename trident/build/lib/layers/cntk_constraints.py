...
def  default_constrains(w:C.Parameter):
    w=C.clip(w,-1e5,1e5)
    return w

def  max_norm(w:C.Parameter ,max_value=2, axis=0):
    w=default_constrains(w)
    norms = C.sqrt(C.sum(C.square(w), axis, keepdims=True))
...
def  min_max_norm(w:C.Parameter,min_value=0.0, max_value=1.0, rate=1.0, axis=0):
    w = default_constrains(w)
    norms = C.sqrt(C.sum(C.square(w), axis, keepdims=True))
