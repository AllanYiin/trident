...
from ..layers.tensorflow_activations import get_activation,Identity
from ..layers.tensorflow_pooling import get_pooling,GlobalAvgPool2d,MaxPool2d
from ..layers.tensorflow_normalizations import get_normalization
...
def ResNet(block, layers, input_shape=(3, 224, 224), num_classes=1000, use_bias=False, zero_init_residual=False,
    ...
    def _make_layer(block, num_filters, blocklayers, strides=1, dilate=False,use_bias=use_bias,layer_name=''):
        ...
        laters_block=Sequential(*layers,name=layer_name)
        return laters_block
...
#         recovery_model=tf.keras.load(os.path.join(dirname,'resnet50.pth'))
